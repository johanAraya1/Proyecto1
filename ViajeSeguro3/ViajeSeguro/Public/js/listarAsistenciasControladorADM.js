    ...
    for (let i = 0; i < listaSolicitudesAsistencia.length; i++) {
        ...
        containerAsistenciaBtnEliminar.addEventListener("click", () => {
            Swal.fire({
                ...
                if (result.value) {
                    eliminarAsistencia(listaSolicitudesAsistencia[i]['_id']);
                    Swal.fire(
        ...
        if (listaSolicitudesAsistencia[i]["estado"] == "activo") {
    ...
    if (filtro != "") {
        for (let i = 0; i < listaSolicitudesAsistencia.length; i++) {
            ...
            if (coincidencias == false) {
                document.querySelector("#resultadoFiltrar").innerHTML = "No se han encontrado coincidencias";
            } else {
