...
const divTituloCat = document.querySelector('#especializadoCat');
...
let mostrar_perfil = async() => {
    ...
    for (let k = 0; k < perfilUsuarioInfo.length; k++) {
        if (localStorageMongoIdUsuarios === perfilUsuarioInfo[k]._id) {
            if (perfilUsuarioInfo[k].tipoDeUsuario !== 'Usuario especializado') {
                divTituloCat.classList.add('displayNone');
            }
            ...
            if (perfilUsuarioInfo[k].tipoDeUsuario !== 'Usuario especializado') {
                botoneditarCategorias.classList.add('displayNone');
            }
        }
    }
}
